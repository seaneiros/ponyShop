  ...
  .controller('AppController', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.selectPonies = function(item) {
      return (
        ...
        (!$scope.filterValues.kind.length || $scope.filterValues.kind.indexOf(item.kind) >= 0) &&
        (item.price >= $scope.filterValues.priceMin) &&
        (item.price <= $scope.filterValues.priceMax) &&
        (item.is_new === ($scope.filterValues.is_new  ? true : item.is_new))
      );
    };


    /**
     * Здесь должен был бы быть вызов сервиса,
     * но чтобы не привязываться к серверу, просто
     * захардкодим исходные данные
     */
